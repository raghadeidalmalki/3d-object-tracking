...
void matchBoundingBoxes(std::vector<cv::DMatch> &matches, std::map<int, int> &bbBestMatches, DataFrame &prevFrame, DataFrame &currFrame)
{
  ...
  /////////////////////
    ...
    // Create a map to store the number of matches for each bounding box pair
    std::map<int, std::map<int, int>> matchCounts;

    // Lambda function to compare if a keypoint is in a bounding box
    auto keypointInBoundingBox = [](const std::vector<cv::KeyPoint> &keypoints, const cv::KeyPoint &kp) -> bool {
        return std::any_of(keypoints.begin(), keypoints.end(), [&kp](const cv::KeyPoint &k) {
            return k.pt == kp.pt; // Compare using keypoint coordinates
        });
    };
    ...
    for (const auto &match : matches)
    {
        ...
        // Find the bounding box ID for the previous frame
        for (const auto &bb : prevFrame.boundingBoxes)
        {
            if (keypointInBoundingBox(bb.keypoints, prevFrame.keypoints[match.queryIdx]))
            {
                prevBBId = bb.boxID;
                break;
            }
        ...
        // Find the bounding box ID for the current frame
        for (const auto &bb : currFrame.boundingBoxes)
        {
            if (keypointInBoundingBox(bb.keypoints, currFrame.keypoints[match.trainIdx]))
            {
                currBBId = bb.boxID;
                break;
            }
        ...
        // Increment the count for this pair of bounding boxes
        if (prevBBId >= 0 && currBBId >= 0)
        {
            matchCounts[prevBBId][currBBId]++;
        }
    }
    ...
    for (const auto &pair : matchCounts)
    {
        ...
        for (const auto &countPair : pair.second)
        {
            if (countPair.second > maxCount)
            {
                bestMatch = countPair.first;
        ...
        if (bestMatch >= 0)
        {
            bbBestMatches[pair.first] = bestMatch;
        }
    }
